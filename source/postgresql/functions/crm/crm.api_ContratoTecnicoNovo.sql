CREATE OR REPLACE FUNCTION crm.api_ContratoTecnicoNovo(a_objeto crm.TContratoTecnicoNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.Negocio IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "Negocio" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
	...
	IF (a_objeto.InicioVigencia IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "InicioVigencia" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;


	IF (a_objeto.Ativo IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "Ativo" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
