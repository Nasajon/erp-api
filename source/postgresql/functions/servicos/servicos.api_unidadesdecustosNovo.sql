CREATE OR REPLACE FUNCTION servicos.api_unidadesdecustosNovo(a_objeto servicos.TunidadesdecustosNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.created_by IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "created_by" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
