CREATE OR REPLACE FUNCTION Servicos.api_ProjetoNovo(a_objeto Servicos.TProjetoNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.TipoProjeto IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "TipoProjeto" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
