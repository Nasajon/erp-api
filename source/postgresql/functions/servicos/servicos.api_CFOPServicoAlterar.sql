CREATE OR REPLACE FUNCTION servicos.api_CFOPServicoAlterar(a_objeto servicos.TCFOPServicoAlterar) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IBGEMunicipio_Anterior IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IBGEMunicipio_Anterior" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
	...
	IF (a_objeto.IBGEMunicipio_Novo IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IBGEMunicipio_Novo" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
