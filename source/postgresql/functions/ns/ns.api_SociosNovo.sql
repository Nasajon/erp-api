CREATE OR REPLACE FUNCTION ns.api_SociosNovo(a_objeto ns.TSociosNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdSocio IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdSocio" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
