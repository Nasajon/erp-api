CREATE OR REPLACE FUNCTION persona.api_DocumentoDependenteFuncionarioAlterar(a_objeto persona.TDocumentoDependenteFuncionarioAlterar) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.Url IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "Url" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;


	IF (a_objeto.Bin IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "Bin" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
