CREATE OR REPLACE FUNCTION persona.api_MovimentoEstabelecimentoAlterarPeriodoSemanal(a_objeto persona.TMovimentoEstabelecimentoAlterarPeriodoSemanal) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.Semana IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "Semana" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
