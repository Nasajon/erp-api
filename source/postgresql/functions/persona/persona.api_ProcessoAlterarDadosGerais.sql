CREATE OR REPLACE FUNCTION persona.api_ProcessoAlterarDadosGerais(a_objeto persona.TProcessoAlterarDadosGerais) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.Processo IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "Processo" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
