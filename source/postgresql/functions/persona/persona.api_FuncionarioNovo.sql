CREATE OR REPLACE FUNCTION persona.api_FuncionarioNovo(a_objeto persona.TFuncionarioNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdFuncionario IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdFuncionario" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
