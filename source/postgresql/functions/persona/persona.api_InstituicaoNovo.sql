CREATE OR REPLACE FUNCTION persona.api_InstituicaoNovo(a_objeto persona.TInstituicaoNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdInstituicao IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdInstituicao" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
