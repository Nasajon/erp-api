CREATE OR REPLACE FUNCTION persona.api_DependenteFuncionarioNovo(a_objeto persona.TDependenteFuncionarioNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdDependente IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdDependente" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
