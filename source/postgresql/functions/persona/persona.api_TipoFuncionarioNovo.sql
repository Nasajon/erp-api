CREATE OR REPLACE FUNCTION persona.api_TipoFuncionarioNovo(a_objeto persona.TTipoFuncionarioNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdTipo IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdTipo" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
