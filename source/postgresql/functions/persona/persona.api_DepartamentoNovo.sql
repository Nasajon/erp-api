CREATE OR REPLACE FUNCTION persona.api_DepartamentoNovo(a_objeto persona.TDepartamentoNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdDepartamento IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdDepartamento" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
