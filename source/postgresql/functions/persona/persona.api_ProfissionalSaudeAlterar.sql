CREATE OR REPLACE FUNCTION persona.api_ProfissionalSaudeAlterar(a_objeto persona.TProfissionalSaudeAlterar) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.Profissional IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "Profissional" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
