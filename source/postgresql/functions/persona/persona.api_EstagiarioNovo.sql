CREATE OR REPLACE FUNCTION persona.api_EstagiarioNovo(a_objeto persona.TEstagiarioNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdEstagiario IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdEstagiario" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
	...
	IF (a_objeto.DataInicioContrato IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "DataInicioContrato" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;


	IF (a_objeto.DataFimContrato IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "DataFimContrato" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
