CREATE OR REPLACE FUNCTION persona.api_FaltaFuncionarioAlterar(a_objeto persona.TFaltaFuncionarioAlterar) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.AbateNoCalculoDoPonto IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "AbateNoCalculoDoPonto" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
