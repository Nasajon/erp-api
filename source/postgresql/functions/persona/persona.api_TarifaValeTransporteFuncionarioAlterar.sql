CREATE OR REPLACE FUNCTION persona.api_TarifaValeTransporteFuncionarioAlterar(a_objeto persona.TTarifaValeTransporteFuncionarioAlterar) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.TarifaConcessionaria IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "TarifaConcessionaria" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
