CREATE OR REPLACE FUNCTION persona.api_FuncionarioAlterarDadosJornada(a_objeto persona.TFuncionarioAlterarDadosJornada) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.Quantidademediahorassemanais IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "Quantidademediahorassemanais" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
