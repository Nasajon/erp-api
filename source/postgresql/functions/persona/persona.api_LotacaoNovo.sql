CREATE OR REPLACE FUNCTION persona.api_LotacaoNovo(a_objeto persona.TLotacaoNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdLotacao IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdLotacao" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
	...
	IF (a_objeto.CodigoAgenteNocivo IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "CodigoAgenteNocivo" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
