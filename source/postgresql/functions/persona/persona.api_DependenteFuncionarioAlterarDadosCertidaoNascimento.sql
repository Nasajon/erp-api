CREATE OR REPLACE FUNCTION persona.api_DependenteFuncionarioAlterarDadosCertidaoNascimento(a_objeto persona.TDependenteFuncionarioAlterarDadosCertidaoNascimento) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.DataNascimento IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "DataNascimento" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
