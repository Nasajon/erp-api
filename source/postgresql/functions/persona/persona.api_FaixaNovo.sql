CREATE OR REPLACE FUNCTION persona.api_FaixaNovo(a_objeto persona.TFaixaNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdFaixa IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdFaixa" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
