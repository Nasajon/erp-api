CREATE OR REPLACE FUNCTION persona.api_AfastamentoFuncionarioAlterarDadosGerais(a_objeto persona.TAfastamentoFuncionarioAlterarDadosGerais) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.DataNova IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "DataNova" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;


	IF (a_objeto.Dias IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "Dias" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
