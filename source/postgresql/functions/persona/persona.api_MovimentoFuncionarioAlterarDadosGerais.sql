CREATE OR REPLACE FUNCTION persona.api_MovimentoFuncionarioAlterarDadosGerais(a_objeto persona.TMovimentoFuncionarioAlterarDadosGerais) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.LotacaoNova IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "LotacaoNova" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
