CREATE OR REPLACE FUNCTION persona.api_SindicatoNovo(a_objeto persona.TSindicatoNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdSindicato IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdSindicato" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
	...
	IF (a_objeto.PisoSalarial IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "PisoSalarial" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
