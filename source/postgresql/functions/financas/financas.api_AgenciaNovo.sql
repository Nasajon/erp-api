CREATE OR REPLACE FUNCTION financas.api_AgenciaNovo(a_objeto financas.TAgenciaNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.IdAgencia IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "IdAgencia" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
