CREATE OR REPLACE FUNCTION contabil.api_lotelancamentoNovo(a_objeto contabil.TlotelancamentoNovo) RETURNS ns.TRecibo AS
$BODY$
DECLARE
  -- BEGIN DECLARE
...
BEGIN      
  BEGIN
	...
	IF (a_objeto.contadebito IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "contadebito" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;


	IF (a_objeto.contacredito IS NULL) THEN
      VAR_RETURN.MENSAGEM := ns.api_montamensagemerro('W0001#O campo "contacredito" não pode ser nulo.');
	  RETURN VAR_RETURN;
	END IF;
